...
Usage:
  train_tex.py [options] <fold-id> <train-dir>
  train_tex.py -h | --help
  train_tex.py --version

Arguments:
  <fold-id>            A folder with train, validation, test split files.
...
from dataset.texture_dataset import TextureDataset
...
def main(args: dict[str, Optional[Any]]):
    ...
    fold_id = Path(args['<fold-id>'])

    train_patients_filepath = fold_id / 'train.txt'
    with open(train_patients_filepath, 'r') as f:
        train_ids = f.readlines()
    train_ids = [int(x.rstrip()) for x in train_ids]

    valid_patients_filepath = fold_id / 'valid.txt'
    ...
    train_dataset = TextureDataset(root_path=train_dir,
                                   patients=train_ids,
                                   categories=categories,
                                   augment=False)
    valid_dataset = TextureDataset(root_path=train_dir,
                                   patients=valid_ids,
                                   categories=categories,
                                   augment=False)
    ...
    test_patients_filepath = fold_id / 'test.txt'
    ...
    if args['--test-dir']:
        ...
        test_dir = train_dir
    test_dataset = TextureDataset(root_path=test_dir, patients=test_ids, categories=categories, augment=False)

    # torch.manual_seed(0)

    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')

    variant = 'efficient_net-pretrained'
    num_cls = len(categories)
    network = create_network(variant, num_cls)

    if args['--weights']:
        network.load_state_dict(torch.load(Path(args['--weights'])))
    ...
    # lrlmbd = lambda epoch: 1.2 if epoch < 10 else 0.99
    # lr_scheduler = MultiplicativeLR(optimizer, lr_lambda=lrlmbd)  # lrsched=3

    # lrlmbd = lambda epoch: 1 if epoch < 10 else 1
    ...
    final_test(network, device, test_dataset, compute_cm=False, verbose=True)


if __name__ == '__main__':
    main(docopt(__doc__, version='train_tex.py 0.1.0'))
